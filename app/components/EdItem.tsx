...
const EdItem = ({image, title, author, review, reviewName, link}: EdItemProps) => {
  ...
  return (
    <div className='m-2 md:w-96'>
        <Image className='border border-white' src={image} alt={title} width={300} height={400}/>
        ...
        <div className='flex'>
            <button className='border border-white rounded-md p-2 text-xs mr-2'>Saber más</button>
            <button className='border border-white rounded-md p-2 text-xs'>Comprar</button>
        </div>
    </div>
  )
}
